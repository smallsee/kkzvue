    ...
    <Row v-if="this.formValidate.is_new" >
      <i-col span="11">
        <Form-item prop="update_date" label="更新时间">
          <Input v-model="formValidate.update_date" placeholder="请输入更新时间"></Input>
        </Form-item>
      </i-col>
    </Row>
...
<script>
  ...
  import {getAkiraList,getTagList, postUpdateVideoList} from 'api/video';
    ...
    data () {
      return {
        ...
        ruleValidate: {
          ...
          update_date: [
            { required: true, message: '请说明更新时期', trigger: 'blur' }
          ],
          introduction: [
    ...
    methods: {
      ...
      handleSubmit (name) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            postUpdateVideoList(formData).then((res) => {
              if (res.meta.errno === ERR_OK){
                this.formValidate.thumb = '';
