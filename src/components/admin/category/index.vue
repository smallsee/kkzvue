...
<script>
  ...
  import {getCategoryList,deleteCategory} from 'api/shop';
  ...
  export default {
    ...
    methods: {
      ...
      update (index) {
        ...
        this.$router.push({ name: '分类修改页', params: { formValidate: this.data[index] }})
      },
      openDelModel (index) {
        this.delModel = true;
        this.readyDel = index;
      ...
      del() {
        deleteCategory(this.data[this.readyDel].id).then(res => {
          if (res.meta.errno === ERR_OK){
